...
def build_kernel(root, kernel):
    exe = os.path.join(root, "bin", f"spmv_{kernel}")
...
def run_with_ncu(ncu, exe, matrix_path, warmup, repeat, root,
                 log_dir, kernel_regex=None, extra_metrics=None):
    ensure_dir(log_dir)
    ...
    for i, row in enumerate(rows):
        if len(
                row) >= 3 and row[0] == "ID" and "Metric Name" in row and "Metric Value" in row:
            header_idx = i
            break
    metrics_map = {}
...
def run_kernel_once(exe, matrix, warmup, repeat, root):
    r = subprocess.run([exe, matrix, str(warmup), str(repeat)],
...
def main():
    ...
    ap.add_argument("--kernel", default="baseline")
    ...
    if not paths:
        raise SystemExit(
            f"no datasets resolved under {data_dir} (inputs: {
                args.paths})")

    exe = build_kernel(root, args.kernel)
    ...
    header = None
    for pth in paths:
        out = run_kernel_once(exe, pth, args.warmup, args.repeat, root).strip()
    ...
    try:
        ...
        piv = df.pivot_table(index="matrix", columns="method", values="gflops")
        cols = [
            c for c in [
                "cusparse_default",
                "cusparse_csr_adaptive",
                f"kernel_{
                    args.kernel}"] if c in piv.columns]
        if cols:
            ax = piv[cols].plot(kind="bar")
            ...
            ax.get_figure().savefig(
                os.path.join(
                    ...
                    f"gflops_{
                        args.kernel}.png"),
                dpi=300)
        if all(c in piv.columns for c in [
               "cusparse_default", "cusparse_csr_adaptive", f"kernel_{args.kernel}"]):
            best = piv[["cusparse_default",
                        "cusparse_csr_adaptive"]].max(axis=1)
            sp = piv[f"kernel_{args.kernel}"] / best
            ax2 = sp.plot(kind="bar")
            ax2.set_ylabel("Speedup vs best cuSPARSE")
            ax2.axhline(1.0)
            ax2.get_figure().tight_layout()
            ax2.get_figure().savefig(
                os.path.join(
                    kernel_out_dir,
                    f"speedup_vs_bestcusparse_{
                        args.kernel}.png"),
                dpi=300)
    ...
    ncu_logs_dir = None
    if ncu:
        ncu_logs_dir = ensure_dir(os.path.join(kernel_out_dir, "ncu_logs"))
        for pth in paths:
            met, _ = run_with_ncu(
                ncu=ncu,
                exe=exe,
                ...
                repeat=args.repeat,
                root=root,
                log_dir=ncu_logs_dir,
                kernel_regex=args.ncu_kernel,
                extra_metrics=args.ncu_metrics,
            )
            ncu_rows.append(met)
        if ncu_rows:
            def to_num(x):
    ...
    if args.dump_ptx_sass:
        if cuobjdump_available():
            dump_dir = ensure_dir(os.path.join(kernel_out_dir, "objdump"))
            ...
            sassptx_csv = os.path.join(
                kernel_out_dir, f"sass_ptx_{
                    args.kernel}.csv")
    ...
    print(out_csv)
    if ncu_rows:
        print(os.path.join(kernel_out_dir, f"ncu_metrics_{args.kernel}.csv"))
        if ncu_logs_dir:
            print(ncu_logs_dir)
    if sassptx_csv:
        print(sassptx_csv)
